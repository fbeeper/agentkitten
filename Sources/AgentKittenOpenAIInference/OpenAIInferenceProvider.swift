...
///
/// Also compatible with any OpenAI-spec endpoint such as LM Studio, Ollama, or other
...
/// key from the supplied ``APIKeyProviding`` at the start of the first turn and caches
...
public actor OpenAIInferenceProvider: InferenceProviding {
    ...
    public init(
        credentials: any APIKeyProviding = EnvironmentAPIKeyProvider("OPENAI_API_KEY"),
        model: String = "gpt-4o",
    ...
    ///
    /// OpenAI applies tool availability at the API-request level on each turn, so
    /// a `toolSelection` change never requires rebuilding the session.
