...
/// by ``InferenceSession/run(_:parameters:)``. The stream always terminates
...
public enum InferenceEvent<Output: Sendable>: Sendable {
    ...
    /// emitting the paired ``toolCallCompleted(id:name:outcome:)``.
    ...
    ///
    /// The conversation layer surfaces the outcome as an agent event and the owning
    ...
    /// the turn trace sink — it is never surfaced to the client event stream.
